...
namespace Dragonwize\PhpCodeQuality\PhpCsFixer;
...
final class TokenRemover
{
    ...
    {
        return !self::hasMeaningTokenInLineBefore($tokens, $index) && !self::hasMeaningTokenInLineAfter(
            $tokens,
            $index
        );
    }
    ...
    private static function hasMeaningTokenInLineBefore(Tokens $tokens, int $index): bool
    {
        ...
        if ($tokens[$prevIndex]->isGivenKind(\T_OPEN_TAG) && !Preg::match(
            '/\\R$/',
            $tokens[$prevIndex]->getContent()
        )) {
            return true;
        }
        ...
        if (!Preg::match('/\\R/', $tokens[$prevIndex]->getContent())) {
            ...
            if (!$tokens[$prevPrevIndex]->isGivenKind(\T_OPEN_TAG) || !Preg::match(
                '/\\R$/',
                $tokens[$prevPrevIndex]->getContent()
            )) {
                return true;
            }
        }
